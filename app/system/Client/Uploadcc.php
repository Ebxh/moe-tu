 ...
 * https://upload.cc/
 */
class Uploadcc extends Client implements ClientInterface
{

    private $name = 'Uploadcc';

    private $upload_api = 'https://upload.cc/image_upload';

    private $image_url = 'https://upload.cc/';
    ...
    public function upload(string $key, string $file): string
    {
        $return_url = '';
        $options = array(
            'uploaded_file[]'    =>  new \CURLFile($file)
        );
        ...
        try {
            ...
            if ($this->http->curl_status_code == 200){
                ...
                if ($data['code'] == 100 && $data['total_success'] == 1 && $data['total_error'] == 0){
                    $success_image = $data['success_image'][0];
                    $url = $this->image_url . $success_image['url'];
