 ...
 */
class Uploadouliu extends Client implements ClientInterface
{

    private $name = 'Uploadouliu';

    private $upload_api = 'https://upload.ouliu.net/';
    ...
    public function upload(string $key, string $file): string
    {
        ...
        try {
            ...
            if ($this->http->curl_status_code == 200){
                ...
                $html->load($result);
                try{
                    $url = $html->find('#thumblink', 0)->value;
                    if (!empty($url)){
